...
bool Heap::insert(void* data)
{
	...
	index = this->get_size() - 1;											// 2. 새 노드의 인덱스 계산

	this->set_treeNode(index, data);										// 3. 새 노드의 데이터를 현재 인덱스에 저장

	this->Heapify_up();														// 4. 상향식으로 재힙화
...
void Heap::Heapify_up()
{
	...
	currIdx = this->get_size() - 1;											// 1. 새 노드의 인덱스와 부모 노드 인덱스 계산
	parentIdx = this->get_parentIdx(currIdx);

																			// 2. 새 노드(leaf)가 부모 노드보다 크면 상향식으로 힙 속성 유지 (최대힙 기준)
	while ((currIdx > 0) &&
		(this->compareFunc(this->get_treeNode(currIdx), this->get_treeNode(parentIdx)) == GREATER))
		// 현재 노드가 부모 노드보다 크면 위로 올라가야 한다. (최대힙 기준)
	{
		this->swapTreeNodes(currIdx, parentIdx);							// 3. 현재 노드와 부모 노드의 내용을 교환

		currIdx = parentIdx;												// 4. 현재 노드 인덱스를 targetIdx로 갱신 (상향식)
		parentIdx = this->get_parentIdx(currIdx);
	}
}

void Heap::Heapify_down()	// 하향식 재힙화 과정 (설명기준 : 최대힙)
{
